...
static errno_t init_module_CLI()
{
    ...
    RegisterCLIcommand(
        ...
        "<imagein> <imageref> <imageout> <xcenteringstart> <ycenteringstart> <xcenteringsize> <ycenteringsize> <sharedmemmode> <semtrig>",
        "imcenternorm imin imref imout 100 100 20 20 0 0",
        "IMG_REDUCE_centernormim(const char* IDin_name, const char *IDref_name, const char *IDout_name, long xcent0, long ycent0, long xcentsize, long ycentsize, int mode, int semtrig);");
...
)
{
    ...
    for(ii = 0; ii < xysize; ii++)
    {
        ...
        data.image[IDrms].array.F[ii] = sqrt(data.image[IDrms].array.F[ii] -
...
)
{
    ...
    while(left != 0)
    {
                ...
                if(data.image[IDbadpix].array.F[jj * xsize + ii] > 0.5)
                {
                    ...
                    for(i = 0; i < 3; i++)
                        for(j = 0; j < 3; j++)
                        {
                            for(kk = 0; kk < zsize; kk++)
                            {
                                pix[kk * 9 + j * 3 + i] = data.image[IDin].array.F[kk * xysize +
                                                          (jj - 1 + j) * xsize + (ii - 1 + i)];
                            }
                    ...
                    if(sum_bpix > 2.1)
                    {
                        for(kk = 0; kk < zsize; kk++)
                        {
                            data.image[IDin].array.F[kk * xysize + jj * xsize + ii] = sum_pix[kk] /
                                    sum_bpix;
                        }
        ...
        for(jj = 1; jj < ysize - 1; jj++)
            for(ii = 1; ii < xsize - 1; ii++)
            {
                data.image[IDbadpix].array.F[jj * xsize + ii] = data.image[IDbadpix1].array.F[jj
                        * xsize + ii];
            }
...
long IMG_REDUCE_cleanbadpix_fast_precompute(
                        ...
                        for(jj1 = jj - distmax; jj1 < jj + distmax + 1; jj1++)
                        {
                                    ...
                                    && (data.image[IDbadpix].array.F[jj1 * xsize + ii1] < 0.5))
                            {
                                if((ii1 != ii) || (jj1 != jj))
                                {
                                    ...
                                    nearbypix_array_dist2[k] = (float)(1.0 * (ii1 - ii) * (ii1 - ii) + 1.0 *
                                                                       (jj1 - jj) * (jj1 - jj));
        ...
        for(kk = 0; kk < zsize; kk++)
        {
            ...
            for(k = 0; k < badpixclean_NBop; k++)
            {
                ...
                data.image[IDout].array.F[kk * xysize + badpixclean_array_indexout[k]] +=
                    badpixclean_array_coeff[k] * data.image[IDout].array.F[kk * xysize +
                            badpixclean_array_indexin[k]];
                //  printf("\n");
                //  fflush(stdout);
            }
        }
...
errno_t IMG_REDUCE_correlMatrix(
    const char *IDin_name,
    const char *IDmask_name,
    ...
    while(loopOK == 1)
    {
        ...
        for(ii = 0; ii < xcentsize; ii++)
            for(jj = 0; jj < ycentsize; jj++)
            {
                ...
                data.image[IDcent].array.F[jj * xcentsize + ii] = data.image[IDin].array.F[jj0 *
                        xsize + ii0];
            }
        ...
        vmin = 1.0;
        for(ii = xsizez / 2 - brad * zfactor; ii < xsizez / 2 + brad * zfactor + 1;
                ii++)
            for(jj = ysizez / 2 - brad * zfactor; jj < ysizez / 2 + brad * zfactor + 1;
                    jj++)
            {
        ...
        for(ii = xsizez / 2 - brad * zfactor; ii < xsizez / 2 + brad * zfactor + 1;
                    ...
                    jj++)
            {
                ...
                data.image[IDcorrz].array.F[jj * xsizez + ii] = pow(
                            data.image[IDcorrz].array.F[jj * xsizez + ii], 2.0);
            }
        ...
        else
        {
            ...
            for(ii = 0; ii < xsize; ii++)
                for(jj = 0; jj < ysize; jj++)
                {
                    data.image[IDout].array.F[jj * xsize + ii] = data.image[IDtout].array.F[jj *
                            xsize + ii];
                }
            data.image[IDout].md[0].write = 0;
            data.image[IDout].md[0].cnt0++;
...
errno_t IMG_REDUCE_cubeprocess(
    const char *IDin_name
)
{
    imageID IDin;
    ...
    if((IDdark = image_ID("calib_dark")) != -1)
    {
        ...
        for(kk = 0; kk < zsize; kk++)
        {
            for(ii = 0; ii < xysize; ii++)
            {
                data.image[IDin].array.F[kk * xysize + ii] -= data.image[IDdark].array.F[kk1 *
                        xysize + ii];
            }
            kk1 ++;
            if(kk1 == zsized)
